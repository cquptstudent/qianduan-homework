    ...
    if (isLoggedIn) {
        if (geDanListByUser.playlist.length > 1) {
            for (let i = 0; i < geDanListByUser.playlist.length; i++) {
                let addElement = "<a class=\"not_clicked_or_not_in_3 not_favorite_3\" href=\"../src/html/ge_dan_detail.html\">\n</a>"
...
function directToGeDan() {
    for (let i = 0; i < divUsedForButton.length; i++) {
        divUsedForButton[i].onclick = function () {
            window.open(encodeURI("./ge_dan_detail.html?" + "listId=" + informationOfGeDan.result[i].id))
        }
    }
}